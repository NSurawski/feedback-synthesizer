  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <span className="w-24 shrink-0 text-gray-500 dark:text-gray-400">
        {neg > 0 && <span className="text-red-500 dark:text-red-400">{neg}%</span>}
        {neg > 0 && neu > 0 && ' · '}
        {neu > 0 && <span className="text-gray-400 dark:text-gray-500">{neu}%</span>}
        {(neg > 0 || neu > 0) && pos > 0 && ' · '}
        {pos > 0 && <span className="text-emerald-500 dark:text-emerald-400">{pos}%</span>}
      </span>
    </div>
  );
}
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md dark:border-gray-700 dark:bg-gray-900">
      <div className="mb-3 flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-gray-900 capitalize dark:text-gray-100">{theme.name}</h3>
      ...
      <div className="mb-3">
        <p className="mb-1 text-xs font-medium text-gray-400 uppercase dark:text-gray-500">Sentiment</p>
        <SentimentBar sentiment={theme.sentiment} />
      </div>

      {theme.quotes.length > 0 && (
        <div className="mb-3">
          <p className="mb-2 text-xs font-medium text-gray-400 uppercase dark:text-gray-500">Top quotes</p>
          <div className="space-y-2">
            {theme.quotes.map((q, i) => (
              <div key={i} className="flex gap-2 rounded-lg bg-gray-50 p-3 text-sm text-gray-700 dark:bg-gray-800 dark:text-gray-300">
      ...
      {expanded && (
        <div className="mt-3 max-h-60 space-y-1 overflow-y-auto rounded-lg border border-gray-100 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800">
          {theme.entries.map((entry) => (
            <div key={entry.id} className="flex items-start gap-2 py-1 text-sm text-gray-600 dark:text-gray-400">
              <span
